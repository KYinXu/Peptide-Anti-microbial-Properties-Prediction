"""SVM-specific sequence inference support."""
...
from ..utils.descriptor_ablation import null_descriptor_values, parse_null_descriptor_names
from ..utils.sklearn_pickle_compat import alias_legacy_sklearn_modules, prepare_legacy_svm

REPO_ROOT = Path(__file__).resolve().parents[2]
SVM_MINIMAL_DIR = REPO_ROOT / "sequence_to_svm_minimal"
if str(REPO_ROOT) not in sys.path:
    sys.path.insert(0, str(REPO_ROOT))
if str(SVM_MINIMAL_DIR) not in sys.path:
    sys.path.insert(0, str(SVM_MINIMAL_DIR))
...
class SvmSequenceScorer:
    """Scores normalized full sequences with a pickled SVM and external z-score file."""
...
def descriptor_matrix(
    ...
    for record in records:
        values = qsar_descriptors(record.sequence, grar740104_matrix)
...
def qsar_descriptors(sequence: str, grar740104_matrix: dict[str, dict[str, float]] | None) -> dict[str, float]:
    ...
    if grar740104_matrix is None:
        ...
        qso = safe_descriptor_call(lambda: descriptor.GetQSOp(maxlag=30, weight=0.05, distancematrix=grar740104_matrix))
    length = len(sequence)
...
def safe_descriptor_call(call) -> dict[str, float]:
    try:
        return call()
    except Exception:
        return {}
...
def score_scaled_matrix(records: list[SequenceRecord], svm: Any, x_scaled: np.ndarray) -> list[SvmSequencePrediction]:
    ...
    sigma = np.asarray(svm.decision_function(x_scaled)).ravel()
    p_amp = np.asarray(svm.predict_proba(x_scaled))[:, positive_index].ravel()
    return [
        SvmSequencePrediction(
            id=record.id,
            sequence=record.sequence,
            prediction=1 if raw_prediction == positive_class else -1,
            sigma=float(score),
            p_amp=float(probability),
        )
        for record, raw_prediction, score, probability in zip(records, raw_predictions, sigma, p_amp)
    ]
